 ...
 */
	...
	function postali_child() {
		...
		wp_register_style( 'icomoon', 'https://d1azc1qln24ryf.cloudfront.net/152819/DunganLeFevre/style-cf.css?sxkip2', array() );
        ...
        if (is_singular('staff') || is_page_template('page-contact.php')) {
            wp_register_script('tab-scripts', get_stylesheet_directory_uri() . '/assets/js/tabs.min.js',array('jquery'), null, true); 
	...
	function postali_child_register_nav_menus() {
		register_nav_menus(
			array(
                ...
                'footer-utility-nav' => __( 'Footer Navigation - Utility', 'postali' ),
			)
		);
	}
    ...
    add_action( 'pre_get_posts', 'staff_archive' );
    // Show all Projects on Projects Archive Page
    function staff_archive( $query ) {
        if ( !is_admin() && $query->is_main_query() && is_post_type_archive( 'staff' ) ) {
                $query->set( 'posts_per_page', '-1' );
        }
    };


    // code blocks to make ajax work on results
    ...
    add_action('wp_ajax_loadmore_results','results_loadmore_ajax_handler');
    add_action('wp_ajax_nopriv_loadmore_results','results_loadmore_ajax_handler');
    ...
    function testimonials_loadmore_ajax_handler(){
        ...
        <?php get_template_part('blocks/block','testimonials-output'); ?>  
